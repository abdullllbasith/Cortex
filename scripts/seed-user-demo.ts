...
async function seedDemoDataForTenant(tenantId: string, ownerId: string, tenantSlug: string) {
  ...
  await seedAllWorkflowTemplatesForTenant(tenantId, ownerId)
  ...
  if (existingSalesEvents >= 200) {
    const { refreshDemoSalesWindow } = await import('../src/lib/seed/refreshDemoSalesWindow')
    const salesRefresh = await refreshDemoSalesWindow(prisma, tenantId, { maxAgeDays: 1 })
    if (salesRefresh.refreshed) {
      console.log(
        `  ✓ Analytics events refreshed (+${salesRefresh.shiftedDays}d so revenue charts stay current)`,
      )
    } else {
