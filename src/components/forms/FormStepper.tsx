...
export interface FormStepperProps<T extends FieldValues = FieldValues> {
  ...
  /** External loading state while account creation / final submit runs */
  completing?: boolean
  className?: string
}
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(completed)) {
        setCompletedSteps(new Set(completed.filter((i) => typeof i === 'number' && i < step)))
      }
    } catch { /* ignore */ }
    ...
    <div className={cn('flex flex-col gap-6', className)}>
      ...
      <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4">
        ...
        <Button
          ...
          size="sm"
          onClick={goNext}
          loading={advancing || completing}
          disabled={advancing || completing}
        >
          {isLast ? (completing ? 'Creating account…' : 'Complete') : 'Next'}
        </Button>
      </div>
    </div>
  )
