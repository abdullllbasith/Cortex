...
>(({ className, children, error, size = 'md', ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex w-full items-center justify-between gap-2 rounded-md border bg-white px-3 text-base text-slate-900 md:text-sm',
      ...
      size === 'sm' ? 'h-7 text-xs' : size === 'lg' ? 'h-11 text-base md:text-sm' : 'h-10 md:h-9',
      className,
    )}
    {...props}
  >
