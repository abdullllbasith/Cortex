...
const thumbSizes: Record<ToggleSize, { base: string; checked: string }> = {
  sm: { base: 'h-3 w-3 translate-x-0.5', checked: 'translate-x-3.5' },
  md: { base: 'h-4 w-4 translate-x-0.5', checked: 'translate-x-4.5' },
  lg: { base: 'h-5 w-5 translate-x-0.5', checked: 'translate-x-5.5' },
}
...
>(({ label, description, size = 'md', className, id: idProp, disabled, ...props }, ref) => {
  ...
  const { base, checked } = thumbSizes[size]
