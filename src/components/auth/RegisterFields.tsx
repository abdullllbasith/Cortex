    ...
    <Controller
      ...
      render={({ field, fieldState }) => (
        <FormField
          name={name}
          label={label}
          helperText="Your team will access Cortex at cortex.app/your-slug"
          required
          error={
        ...
        >
          <div className="flex w-full min-w-0 items-stretch">
            <span
              className={cn(
                'flex h-9 shrink-0 items-center rounded-l-md border border-r-0 bg-slate-50 px-3 text-sm text-slate-500 dark:bg-slate-900 dark:text-slate-400',
                fieldState.error ||
                  (!fieldState.error && data?.available === false && debouncedSlug?.length >= 3)
            ...
            <div className="relative min-w-0 flex-1">
              <Input
                ...
                className="h-9 w-full rounded-l-none rounded-r-md pr-10"
                error={
                  fieldState.error?.message ??
                  (!fieldState.error && data?.available === false && debouncedSlug?.length >= 3
                    ? 'This slug is already taken'
