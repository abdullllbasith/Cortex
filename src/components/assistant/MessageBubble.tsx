...
export function MessageBubble({ message, onUndoAction }: MessageBubbleProps) {
  ...
  return (
    <div className={cn('flex gap-2 px-3 py-3 sm:gap-3 sm:px-4', isUser ? 'flex-row-reverse' : 'flex-row')}>
      {isUser ? (
        <Avatar
          size="sm"
          name={displayName}
      ...
      <div className={cn('flex min-w-0 max-w-[min(85%,100%)] flex-col sm:max-w-[75%]', isUser ? 'items-end' : 'items-start')}>
        <div
          className={cn(
            'w-fit max-w-full rounded-2xl px-3 py-2.5 text-sm leading-relaxed shadow-sm sm:px-4',
            isUser
        ...
        >
          {message.streaming && !message.content ? (
            ...
            <p className="m-0 break-words whitespace-pre-wrap [overflow-wrap:anywhere]">{message.content}</p>
          ) : (
            <div className="prose prose-sm dark:prose-invert max-w-none break-words [overflow-wrap:anywhere] prose-p:my-1 prose-ul:my-1 prose-ol:my-1 prose-headings:my-2 prose-pre:max-w-full prose-pre:overflow-x-auto prose-pre:bg-slate-900 prose-pre:text-slate-100">
