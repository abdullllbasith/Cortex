...
import { productCreateSchema, paginationSchema } from '@/lib/knowledge/schemas'

export const GET = withTenantAuth(async (request, { auth }) => {
  try {
    const query = paginationSchema.parse(parseQuery(request))
    const { data, total } = await knowledgeRepository.listProducts(auth.tenantId, query)
...
export const POST = withTenantAuth(async (request, { auth }) => {
  try {
    ...
    const record = await knowledgeRepository.createProduct(auth.tenantId, body, auth.userId)
