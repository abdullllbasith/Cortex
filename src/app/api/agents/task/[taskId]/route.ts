    ...
    if (stream && (task.status === 'PENDING' || task.status === 'PROCESSING')) {
      const streamBody = new ReadableStream({
        async start(controller) {
          ...
          // Recover PENDING tasks that never got a worker (serverless / no Redis).
          if (task.status === 'PENDING') {
            const preferred = task.agentType ? PRISMA_TO_AGENT_KEY[task.agentType] : undefined
            void processAgentTaskInline({
              taskId: task.id,
              ...
              console.error('[agents/task stream] inline processing failed', err)
            })
          }
