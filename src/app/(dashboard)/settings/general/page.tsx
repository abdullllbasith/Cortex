...
export default function GeneralSettingsPage() {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      const saved = await apiClient.put<TenantGeneralDTO>('/settings/general', {
        name: values.name,
        settings: {
          logoUrl: logo || null,
      ...
      useSessionStore.getState().updateTenant({
        ...
        primaryColor: saved.settings.primaryColor ?? undefined,
        secondaryColor: saved.settings.secondaryColor ?? undefined,
      })
      ...
      notifyBrandingUpdated({
        ...
        primaryColor: saved.settings.primaryColor,
        secondaryColor: saved.settings.secondaryColor,
      })
      void mutate(saved, { revalidate: false })
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Failed to save workspace settings')
    }
  }
    ...
    <FormProvider {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col">
        ...
        <div className="max-w-3xl space-y-6 p-6 pb-10">
          <FormSection title="Workspace Identity" description="How your workspace appears across Cortex">
            <FormFileUpload
              name="logoUrl"
