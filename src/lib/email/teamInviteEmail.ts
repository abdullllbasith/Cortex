...
export async function sendTeamInviteEmail(opts: {
  ...
  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:560px;margin:0 auto;color:#0f172a">
      <h2 style="font-size:20px;margin-bottom:8px">Join ${opts.tenantName} on SAIOS</h2>
      <p style="color:#475569;line-height:1.6">${opts.message?.trim() || "You've been invited to join our workspace on SAIOS."}</p>
  ...
  await sendMail({
    to: opts.to,
    subject: `Join ${opts.tenantName} on SAIOS`,
    text,
    html,
  })
