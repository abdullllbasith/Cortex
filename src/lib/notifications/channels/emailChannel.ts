...
export function formatEmailHtml(title: string, body: string, actionUrl?: string, actionLabel?: string): string {
  ...
  const cta = href
    ? `<p style="margin-top:16px"><a href="${href}" style="background:#4f46e5;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none">${actionLabel ?? 'Open in Cortex'}</a></p>`
    : ''
