...
function platformEmailHtml(title: string, body: string, type: AnnouncementType): string {
  ...
  const cta = `<p style="margin-top:20px"><a href="${process.env.NEXT_PUBLIC_APP_URL ?? 'https://cortex.app'}/dashboard" style="background:${accent};color:#fff;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:600">Open Cortex</a></p>`
  return `<!DOCTYPE html><html><body style="font-family:Inter,sans-serif;color:#0f172a;max-width:560px;margin:0 auto">
    <div style="border-bottom:3px solid ${accent};padding-bottom:12px;margin-bottom:20px">
      <p style="font-size:12px;text-transform:uppercase;letter-spacing:0.05em;color:${accent};margin:0">Cortex Platform</p>
      <h1 style="margin:8px 0 0;font-size:22px">${title}</h1>
    </div>
    <div style="line-height:1.6;color:#334155">${body.replace(/\n/g, '<br/>')}</div>
    ${cta}
    <p style="margin-top:32px;font-size:12px;color:#94a3b8">You received this because you are an owner on a Cortex workspace.</p>
  </body></html>`
}
