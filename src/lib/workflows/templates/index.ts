...
export type WorkflowTemplateSummary = Omit<WorkflowTemplateMeta, 'definition'>

export const CROSS_MODULE_TEMPLATE_IDS = [
  'order-to-cash',
  'procure-to-pay',
  'procure-to-pay-receipt',
  'lead-to-deal',
...
export function listTemplateSummaries(): WorkflowTemplateSummary[] {
  return TEMPLATES.map(({ definition: _definition, ...summary }) => summary)
}
...
export function getTemplateSummaryById(id: string): WorkflowTemplateSummary | undefined {
  const t = TEMPLATES.find((item) => item.id === id)
  if (!t) return undefined
  const { definition: _definition, ...summary } = t
  return summary
}

export function getCrossModuleTemplates(): WorkflowTemplateSummary[] {
  return CROSS_MODULE_TEMPLATES.map(({ definition: _definition, ...summary }) => summary)
}
