...
export const sendNotificationHandler: NodeHandler = async ({ inputData, config }, engineCtx) => {
  ...
  const message = String(ctx.interpolate(cfg.message ?? cfg.template ?? 'Notification from SAIOS workflow'))
  ...
  switch (cfg.channel) {
    case 'slack':
      ...
      break
    case 'email':
      await sendEmail(recipient, 'SAIOS Workflow Notification', message)
      break
    case 'whatsapp':
