...
export function resolveDateRange(
  ...
  switch (period) {
    case 'today': {
      // UTC day bounds — Vercel runs in UTC. Use end-of-day (not `now`) so
      // timestamp-without-time-zone rows stored with later wall-clock hours still match.
      ...
      previousStart = new Date(
        Date.UTC(previousEnd.getUTCFullYear(), previousEnd.getUTCMonth(), previousEnd.getUTCDate()),
      )
      label = 'Today'
      break
    }
