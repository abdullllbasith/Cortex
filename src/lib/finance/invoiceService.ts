...
export async function sendInvoice(invoiceId: string, tenantId: string, actorId?: string) {
  ...
  await sendInvoiceEmail({
    ...
    body: `Please find attached invoice ${invoice.invoiceNumber} from ${tenant?.name ?? 'SAIOS'}. Amount due: ${total} ${invoice.currency}. Due date: ${invoice.dueDate.toLocaleDateString()}.`,
    pdfBuffer,
